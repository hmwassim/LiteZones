...
## LiteZones file inventory

Core: `App`, `Hooks`, `DragController`, `KeyboardSnap`, `TrayService` · Layout: `LayoutEngine`, `Zone`, `LayoutTypes`, `LayoutHelpers`, `LayoutAssignedWindows`, `ZoneAssignmentStore`, `CanvasMath` · Editor: `EditorWindow`, `EditorCanvas`, `GridData` · Overlay: `ZonesOverlay`, `HighlightedZones`, `Colors` · Platform: `WindowUtils`, `WindowProcessing`, `WindowProperties`, `MonitorManager`, `WorkArea`, `WorkAreaManager` · Data: `Settings`, `CustomLayouts`, `AppliedLayouts`, `AppZoneHistory`, `json`, `Paths`, `FileWatcher` · Utils: `GuidUtils` (GUID parsing, layout type strings), `ZoneNavigation` (arrow-key zone navigation).
