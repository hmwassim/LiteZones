...
bool Hooks::Start()
{
    ...
    return m_moveStartHook != nullptr && m_moveEndHook != nullptr;
}
...
LRESULT CALLBACK Hooks::LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    if (nCode == HC_ACTION)
    {
        ...
        if (info->vkCode == VK_SHIFT || info->vkCode == VK_CONTROL)
        {
            const bool pressed = (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN);
            PostMessageW(s_targetWindow, WM_PRIV_KEYSTATE, info->vkCode, pressed ? 1 : 0);
        }
...
LRESULT CALLBACK Hooks::LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    if (nCode == HC_ACTION)
    {
        ...
        {
            const UINT button = (wParam == WM_RBUTTONDOWN) ? VK_RBUTTON : VK_MBUTTON;
            PostMessageW(s_targetWindow, WM_PRIV_MOUSEBUTTON, button, 1);
        }
    }
    return CallNextHookEx(nullptr, nCode, wParam, lParam);
}
